...
namespace Core.Data.Repositories {
    public class Repository<T> : IRepository<T> where T : class, new() {
        ...
        public Repository(ISession session) {
            ...
            // Set the initial default batch size, if nothing is configured, do not set it at all.
            string batchSize = null;// ConfigurationHelper.GetValue(ConfigKeys.NHibernateBatchSize);
            if (batchSize == null) 
                return;
            
            int batch;
            if (int.TryParse(batchSize, out batch))
                SetBatchSize(batch);
        }
        ...
        {
            _session.Query<T>().Where(expression).ForEach(p => _session.Delete(p));
        }
        ...
        {
            return _session.CreateSQLQuery(string.Format(query, parameters)).List<T2>();
        }
